...
function constrainDistance(point, anchor, distance) {
  ...
  return new Vector2D(
    anchor.x + dir.x * scale,
    anchor.y + dir.y * scale
  );
}
...
class Chain {
  constructor(origin, jointCount, linkSize, angleConstraint = Math.PI * 2) {
    ...
    for (let i = 1; i < jointCount; i++) {
      this.joints.push(Vector2D.add(
        this.joints[i - 1], 
        new Vector2D(0, this.linkSize)
      ));
      this.angles.push(0);
    }
  }
  ...
  resolve(pos) {
    ...
    for (let i = 1; i < this.joints.length; i++) {
      const curAngle = Vector2D.sub(this.joints[i - 1], this.joints[i]).heading();
